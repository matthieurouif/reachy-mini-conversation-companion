...
class LookAtSky(Tool):
    """Look at a specific point in the sky given an object name."""

    name = "look_at_sky"
    description = "Look at the sky in the direction of an astronomical object (stars, planets, moon, sun, galaxies, nebulae, clusters)."
    parameters_schema = {
        "type": "object",
        "properties": {
            "astronomical_object": {
    ...
    async def __call__(self, deps: ToolDependencies, **kwargs: Any) -> Dict[str, Any]:
        ...
        result = find_celestial_angles(astronomical_object, latitude, longitude)

        if result['found']:
            print(f"  ✓ Found: {result['object_name']} ({result['type']})")
...
def sky_orientation(deps, altitude_deg, azimuth_deg):
    ...
    SAFE_PREROTATION_Z_LIFT_M = 0.02  # Lift head 3cm vertically before rotating
    ...
    logger.info("Tool call: look_at_sky azimuth=%.1f° altitude=%.1f°", azimuth_deg, altitude_deg_clamped)
